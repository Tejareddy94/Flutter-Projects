...
class MedicalPosts extends StatefulWidget {
  const MedicalPosts({
    Key key,
  }) : super(key: key);
  @override
  _MedicalPostsState createState() => _MedicalPostsState();
}

class _MedicalPostsState extends State<MedicalPosts> {
  bool _isLoading;
  bool _isMore = false;
  int _currentPage = 1;
      ...
      if (bearer?.isNotEmpty ?? false) {
        if (currentPage != null && filter != null) {
          response = await BackendService.get(
              '/api/category_posts/2/?page=$currentPage&$filter',
              {HttpHeaders.authorizationHeader: "Bearer " + bearer},
              context);
        } else {
          response = await BackendService.get(
              '/api/category_posts/2/?page=$currentPage',
              {HttpHeaders.authorizationHeader: "Bearer " + bearer},
              context);
        }
