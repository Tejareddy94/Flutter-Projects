...
class MyDistrictPosts extends StatefulWidget {
  @override
  _MyDistrictPostsState createState() => _MyDistrictPostsState();
}

class _MyDistrictPostsState extends State<MyDistrictPosts> {
  bool _isLoading;
  bool _isMore = false;
  int _currentPage = 1;
  ...
  Future fetchPosts(int currentPage) async {
    ...
    try {
      ...
      final response = await BackendService.get(
          '/api/my_district_posts/?page=$currentPage',
  ...
  Widget build(BuildContext context) {
    ...
    return PickupLayout(
      ...
      scaffold: Scaffold(
        key: scaffoldKey,
        appBar: AppBar(title: Text("My District Posts")),
        body: _isLoading
            ? Column(
                children: <Widget>[
                  ListView.builder(
