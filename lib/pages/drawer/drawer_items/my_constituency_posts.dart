...
class MyConstituencyPosts extends StatefulWidget {
  @override
  _MyConstituencyPostsState createState() => _MyConstituencyPostsState();
}

class _MyConstituencyPostsState extends State<MyConstituencyPosts> {
  bool _isLoading;
  bool _isMore = false;
  int _currentPage = 1;
  ...
  Future fetchPosts(int currentPage) async {
    ...
    try {
      ...
      final response = await BackendService.get(
          '/api/my_constituency_posts/?page=$currentPage',
  ...
  Widget build(BuildContext context) {
    ...
    return PickupLayout(
      ...
      scaffold: Scaffold(
        key: scaffoldKey,
        appBar: AppBar(title: Text("My Constituency Posts")),
        body: _isLoading
            ? Column(
                children: <Widget>[
                  ListView.builder(
